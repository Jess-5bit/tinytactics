...
func update_view():
	...
	match characters[selected].type:
		Game.TYPE.ARCHER:
			animation = "archer"
		Game.TYPE.MAGE:
			animation = "mage"
	$name.text = characters[selected].name
	$atk.text = "ATK" + "%02d" % (characters[selected].attack_damage + characters[selected].item.attack)
	$hp.text = "HP" + "%02d" % characters[selected].hp
	$def.text = "DEF" +  "%02d" % (characters[selected].defense + characters[selected].item.defense)
	$type.play(animation)
